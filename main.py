...
### python main.py --sigma 0.6 --embedding_dim 100 --data_folder /home/jw7jb/seqrec/hawkes/dataset --train_data xing_session_train.pickle --valid_data xing_session_test.pickle --test_data xing_session_test.pickle
...
def main():
	...
	# test_data = os.path.join(args.data_folder, args.test_data)

	### python main.py --sigma 0.6 --embedding_dim 100
	# train_data = "../Data/xing_session_train.pickle"
	# valid_data = "../Data/xing_session_test.pickle"
	# test_data = "../Data/xing_session_test.pickle"
	...
	print("load test data", test_data)

	train_data = dataset.Dataset(train_data)
	valid_data = dataset.Dataset(valid_data)
	...
	if not args.is_eval:
		network = GRU4REC(input_size, hidden_size, output_size, final_act=final_act,
		...
		optimizer = Optimizer(network.parameters(), optimizer_type=optimizer_type, lr=lr, weight_decay=weight_decay, momentum=momentum, eps=eps)

		loss_function = LossFunction(loss_type=loss_type, use_cuda=args.cuda)

		trainer = Trainer(network, train_data=train_data, eval_data=valid_data, optim=optimizer, use_cuda=args.cuda, loss_func=loss_function, args=args)

		trainer.train(writer, 0, n_epochs-1, batch_size)
	else:
		if args.load_model is not None:
			...
			loss_function = LossFunction(loss_type=loss_type, use_cuda=args.cuda)
			evaluation = Evaluation(network, loss_function, use_cuda=args.cuda)
			loss, recall, mrr = evaluation.eval(valid_data)
			print("final result: recall={:.2f}, mrr={:2.f}".format(recall, mrr))
		else:
			print("Pre trained network is None!")
