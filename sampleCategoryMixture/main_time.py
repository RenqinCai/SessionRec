"""
use time threshold to cut sequences
command 
CUDA_VISIBLE_DEVICES=1 python main.py --data_folder ../Data/tmall/100000_unknown_cate/ --data_action item.pickle --data_cate cate.pickle --data_time time.pickle --data_name taobao --embedding_dim 300 --hidden_size 300 --lr 0.0001 --window_size 5 --test_observed 5 --n_epochs 300 --shared_embedding 1 --batch_size 200 --optimizer_type Adam 
"""
...
parser.add_argument('--model_name', default="sampleCategoryMixture", type=str)
...
def main():
	...
	# ss = SampledSoftmax(ntokens, nsampled, 200, tied_weight=None)
	# print(ss.params.weight.data)
	...
	cate_input_size = len(data_obj.cates)
	...
	if not args.is_eval:
		...
		model = GRU4REC(log, ss, window_size, input_size, hidden_size, output_size, cate_embedding_dim=cate_embedding_dim, cate_input_size=cate_input_size, cate_output_size=cate_output_size, cate_hidden_size=cate_hidden_size,
							final_act=final_act,
