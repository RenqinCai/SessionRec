...
class GRU4REC(nn.Module):
    ...
    def forward(self, input_cate_batch, mask_cate_batch, max_actionNum_cate_batch, seqLen_cate_batch, input_batch, mask_batch, seqLen_batch, train_test_flag):
        ...
        pad_seqLen_cate_batch = [i-1 if i > 0 else 0 for i in seqLen_cate_batch]
        first_dim_index = torch.arange(batch_size_cate).to(self.device)
        second_dim_index = torch.LongTensor(pad_seqLen_cate_batch).to(self.device)
        
        output_cate = output_subseq_cate[first_dim_index, second_dim_index, :]
        ...
        ### batch_size*hidden_size
        mixture_output = output_cate+output_short
