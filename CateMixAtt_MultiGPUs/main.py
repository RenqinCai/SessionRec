...
def main():
	...
	cuda_device_name = torch.device('cuda' if args.cuda else 'cpu')

	# print("there are ", torch.cuda.device_count(), " GPUs")

	if not args.is_eval:
		model = GRU4REC(window_size, input_size, hidden_size, output_size, 						device=cuda_device_name,
							final_act=final_act,
		...
		model = model.to(cuda_device_name)
		...
		loss_function = LossFunction(loss_type=loss_type, device=cuda_device_name)
