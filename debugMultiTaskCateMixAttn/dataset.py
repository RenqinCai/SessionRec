...
class Dataset(object):
	...
	def __init__(self, action_file, cate_file, observed_threshold, window_size, itemmap=None):
		...
		self.m_input_cate_subseq_list_cate_list = []
		...
		for seq_index in range(seq_num):
			...
			for action_index in range(actionNum_seq):
				...
				if action_index <= window_size:
					...
					self.m_input_cate_subseq_list_cate_list.append(cate_subseq_list)
					self.m_target_cate_seq_list.append(cate_cur)
		...
		print("cate subseq num", len(self.m_input_cate_subseq_list_cate_list))
		print("target seq num", len(self.m_target_cate_seq_list))
...
class DataLoader():
	...
	def __iter__(self):
		...
		input_cate_subseq_list_cate_list = self.m_dataset.m_input_cate_subseq_list_cate_list
		target_cate_seq_list = self.m_dataset.m_target_cate_seq_list
		...
		for batch_index in range(batch_num):
			...
			for seq_index_batch in range(batch_size):
				...
				pad_cate_subseq_list = input_cate_subseq_list_cate_list[seq_index]
				pad_cate_subseq_list += [0]*(max_subseqNum_cate_batch-len(pad_cate_subseq_list))
				cate_subseq_batch += pad_cate_subseq_list 

				y = target_action_seq_list[seq_index]
				y_batch.append(y)
				idx_batch.append(input_seq_idx_list[seq_index])

				target_cate = target_cate_seq_list[seq_index]
				target_cate_batch.append(target_cate)
			...
			cate_subseq_batch = np.array(cate_subseq_batch)
			...
			### cate for subsequences
			cate_subseq_batch_tensor = torch.from_numpy(cate_subseq_batch)

			### cate for target action
			target_cate_batch_tensor = torch.from_numpy(target_cate_batch)
			...
			yield x_cate_batch_tensor, mask_cate_batch_tensor, mask_cate_seq_batch_tensor, max_actionNum_cate_batch, max_subseqNum_cate_batch, subseqLen_cate_batch, seqLen_cate_batch, cate_subseq_batch_tensor, target_cate_batch_tensor, x_batch_tensor, mask_batch_tensor, seqLen_batch, y_batch_tensor, idx_batch_tensor
