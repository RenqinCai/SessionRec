...
class CTR(nn.Module):
    def __init__(self, log, ss, position_size, input_size, hidden_size, output_size, decay_size, num_heads, num_layers=1, dropout_hidden=.8, dropout_input=0, embedding_dim=-1, use_cuda=True, shared_embedding=True):
        ...
        self.m_position_size = position_size
        ...
        self.m_position_embed = nn.Embedding(self.m_position_size, self.m_embedding_dim)
    ...
    def forward(self, position_self_src, self_src, common_src, common_time, friend_diff_src, friend_num_src, friend_num_src_tensor):

        ### self_src: batch_size*seq_len
        ### common_src: (batch_size*friend_num)*common_num
        ### common_time: (batch_size*friend_num)*common_num
        ### friend_diff_src: (batch_size*friend_num)*diff_num
        ### friend_num_src: batch_size*1

        embed_x = self.m_input_embed(self_src)
        common_x = self.m_input_embed(common_src)
        friend_diff_x = self.m_input_embed(friend_diff_src)

        embed_position = self.m_position_embed(position_self_src)

        embed_x_position = embed_x+embed_position

        if self.m_embedding_dim != self.m_hidden_size:
            embed_x_position = self.m_fc_relu(self.m_fc(embed_x_position))
        ...
        self_x = self.m_self_attn_module(self_src_mask, embed_x_position)
...
class FRIENDSHIP(nn.Module):
    def __init__(self, self_input_size, friend_input_size, friendship_size):
        super(FRIENDSHIP, self).__init__()
        self.m_linear_friend = nn.Linear(friend_input_size, friendship_size, bias=False)
        self.m_linear_beta = nn.Linear(self_input_size, friendship_size, bias=False)
        self.m_tau = 1e5

    def forward(self, common_x, self_x, common_time, friend_num_src, friend_num_src_tensor, common_src_mask):
        ### common_x size: (batch_size*friend_num)*common_num*hidden_size
        ### self_x size: batch_size*hidden_size
        ### common_src_mask size: (batch_size*friend_num)*common_num
        
        ### self_x size: (batch_size*friend_num)*hidden_size
        self_x = torch.repeat_interleave(self_x, repeats=friend_num_src_tensor, dim=0)

        ### self_x size: (batch_size*friend_num)*hidden_size*1
        self_x = self_x.unsqueeze(-1)

        ### content_weight size: (batch_size*friend_num)*common_num*1
        content_friendship = torch.bmm(common_x, self_x)
        ...
        temporal_weight = torch.exp(-common_time/self.m_tau)
        ...
        temporal_friendship = torch.sum(content_temporal_weight, dim=-1)
